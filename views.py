...
def problem_one(request):
    ...
    good_students = Student.objects.filter(gpa__gt=3).order_by('-gpa')
    for smarty_pants in good_students:
      print(f'Full name: {smarty_pants.first_name} {smarty_pants.last_name} GPA: {smarty_pants.gpa}')
...
def problem_two(request):
    ...
    instructor_2010 = Instructor.objects.filter(hire_date__lt='2010-01-01').order_by('hire_date')
    
    for old_instructors in instructor_2010:
      print(f'Full name: {old_instructors.first_name} {old_instructors.last_name} Hire date: {old_instructors.hire_date}')
...
def problem_three(request):
    ...
    instructor_man = Instructor.objects.get(pk=2)
    courses = Course.objects.filter(instructor_id=2)
    
    for the_instructor in instructor_man:
      print(f'Instructor name {the_instructor.first_name} {the_instructor.last_name}')
    for the_courses in courses:
      print(f'Courses: {the_courses.instructor_id}')
...
def problem_four(request):
    ...
    for students_list in students:
      print(f"Students count: {students_list} ")
    for courses_list in courses:
      print(f"Courses count: {courses_list} ")
    for instructors_list in instructors:
      print(f"Instructor count: {instructors_list} ")
...
def problem_five(request):
  student = Student()
  student.first_name = 'Justin'
  student.last_name = 'Goncalves'
  student.year = 1
  student.gpa = 4.0
  student.save()
...
def problem_six(request):
  ...
  student_id = 11
  Student.objects.filter(pk=student_id).update(year=1)
